...
def sample_prior_params(n_param, n_prior_samples, x, data):
	...
	sampled_array[:,0] = np.random.normal(loc = intercept_loc, scale = 1, size = n_prior_samples)
	sampled_array[:,1] = np.random.uniform(low = 0, high = slope_range, size = n_prior_samples)
...
data = np.load('polynom_slope_trends_sigma0.1_2.2_4.5_ws750.npy')[:,:,0]
leave_out_support_index = 1 #### number of first BF support points that is leaved out
...
loop_range = np.arange(0, simulation_length * within_year_points - window_size, window_shift, dtype = int)
...
for j in range(loop_range.size): #### adapt point_of_no_return to make it the index of loop_range
	if window_size - 1 + loop_range[j] >= point_of_no_return:
...
for i in range(sigma_array.size):
	np.random.seed(42)
	time_series = np.load('ecological_model_planktivores_sig' + str(sigma_array[i]) + 'ppyear50.npy')
	# plt.plot(time, time_series)
	# plt.show()

	if control_plot_point_of_no_return:
		plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[0,leave_out_support_index:loop_range[:point_of_no_return].size])
		plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[1,leave_out_support_index:loop_range[:point_of_no_return].size])
		plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[2,leave_out_support_index:loop_range[:point_of_no_return].size])
		plt.show()
	for k in range(MC_runs):
		if print_MC_runs:
			...
			print('__________________________')
			print('__________________________')
		prior_sampling_array = sample_prior_params(n_param, n_prior_samples, time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[i,leave_out_support_index:loop_range[:point_of_no_return].size])
		...
		if control_plots_prior:
			plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[i,leave_out_support_index:loop_range[:point_of_no_return].size], marker = 'o', color = 'r')
			for j in range(n_prior_samples):
				plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], M1(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], prior_sampling_array[j,:]), color = 'grey')
			plt.show()

			plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[i,leave_out_support_index:loop_range[:point_of_no_return].size], marker = 'o', color = 'r')
			for j in range(n_prior_samples):
				plt.plot(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], M2(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], prior_sampling_array[j,:]), color = 'grey')
			plt.show()
		...
		for j in range(n_prior_samples):
			...
			log_LH_model1_array[j] = log_likelihood(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[i,leave_out_support_index:loop_range[:point_of_no_return].size], prior_sampling_array[j,:], M1)
			log_LH_model2_array[j] = log_likelihood(time[window_size - 1 + loop_range[leave_out_support_index:point_of_no_return]], data[i,leave_out_support_index:loop_range[:point_of_no_return].size], prior_sampling_array[j,:], M2)
			if print_log_LH:
